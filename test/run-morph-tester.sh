#!/bin/bash
# test/run-morph-tester.sh.  Generated from run-morph-tester.sh.in by configure.
...
pythonbin=/opt/local/bin/python3.3
runtests="$pythonbin /Users/ttr000/main/giella-core/scripts/morph-test.py"
...
for f in $fsttype; do
	...
	generator=""
	if test $f == "xfst"; then
		lookuptool="/Users/ttr000/bin/lookup -flags mbTT"
		section="xerox"
		suffix="xfst"
		...
		fastlookup="/usr/local/bin/hfst-optimized-lookup"
		section="hfst"
		suffix="hfstol hfst"
	else
	    let "Fail += 1"
